...
required="dataDir readType organismDomain projectDir prokkaGFFannotation".split()
...
parser.add_option('-d', '--organismDomain',
				  ...
				  help="[OPTIONAL] rnaSeq read type. Choose from 'prokaryote' or 'eukaryote' ",
				  dest='organismDomain'
				  )

parser.add_option('-g', '--prokkaGFFannotation',
				  type='choice',
				  choices=['yes', 'no'],
				  help=" If providing GFF file, please specify if GFF is obtained from prokka or not. Choose from 'yes' or 'no'  ",
				  dest='prokkaGFFannotation'
				  )
...
prokkaGFF=option_dict.get('prokkaGFFannotation')
...
def transcriptome(transcriptome_dir):
	...
	for transcriptome in transcriptome_file_list:
		transcriptome_allowed_extn=[".fasta",".fa",".ffn",".fas",".fna"]
...
with open('luigi.cfg', 'w') as config:
	...
	if (len(os.listdir(annotationDir))==0):
		...
		config.write('prokka_gff={prokkaGFF}\n'.format(prokkaGFF=prokkaGFF))
